...
> Rates changed with `/xprate` last until an admin turns them off with `/xprate reset`, the server restarts, or mcMMO's configs are reloaded. They are never written to `experience.yml`. For a permanent XP rate, set `Experience_Formula.Multiplier.Global` in `experience.yml` instead.
{.is-info}
